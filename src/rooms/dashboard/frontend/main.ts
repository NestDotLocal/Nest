...
const openPanel = (): void => {
    ...
    panelList
        .querySelectorAll<HTMLButtonElement>(".widget-panel__item")
        .forEach((btn) => {
...
document
    .getElementById("widget-panel-close")!
...
if (cached) {
    ...
    loadLayout().then((fresh) => {
        ...
        const freshIds = JSON.stringify(
            fresh.map((l: WidgetLayout) => l.id).sort(),
        );
