...
import { buildTree, renderTree, renderSkeleton, getOpenFolders, getSelectedEntry, setSelected, type SidebarOptions } from "@nest/sidebar";
...
export const showNewNoteInput = (sidebar: HTMLElement, entries: any[], onCreated: (fresh: any[]) => void): void => {
    ...
    if (targetFolder !== '/') {
        ...
        if (selected?.type === 'folder') {
            const selectedEl = sidebar.querySelector<HTMLElement>('.entry.folder.selected');
    ...
    const confirm = async () => {
        ...
        try {
            ...
            onCreated(fresh);
            const encodedPath = newEntry.path.split('/').map((s: string) => encodeURIComponent(s)).join('/');
